...
(function (global) {
  ...
  const TYPE_ICON_MAP = {
    ...
    midnight: {
      icon: null,
      label: "Continues to next day",
      class: "ev-midnight-icon",
      symbol: "↪",
    },
  };
  ...
  function renderStatusIcons(
    ...
    if (
      hideForRescheduleCurrent &&
      event.isRescheduleCurrent &&
      !event.isTeacherChanged
    ) {
      return "";
    }
    ...
    if (event.isTeacherChanged) {
      const teacherIcon = renderTeacherChangeIcon(
        event,
        statusMeta,
        { position, top, right, size: 16 },
        currentWeekStart,
      );
      const coveredIcon = renderIcon(
        STATUS_ICON_MAP.covered.icon,
        STATUS_ICON_MAP.covered.label,
        "",
        { size: 16 },
      );
    ...
    if (
      statusMeta.code === "cancel_reschedule_later" &&
      SPECIAL_STATUS_ICONS[statusMeta.code]
    ) {
    ...
    const hasHistory =
      event.history && Array.isArray(event.history) && event.history.length > 0;
    const tooltipId = hasHistory
      ? `status-history-tooltip-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`
      : "";
   ...
   */
  function renderTeacherChangeIcon(
    event,
    statusMeta = null,
    options = {},
    currentWeekStart = null,
    ...
    debugger;
    if (summaryPrevious?.teacher?.id === event.teacherId) {
      teacherPic = summaryPrevious.teacher.avatar;
    } else {
      teacherPic = summaryCurrent.teacher.avatar;
    }
    ...
    newTeacherName =
      summaryCurrent?.teacher?.name ||
      resCurrent?.teacher_name ||
      resCurrent?.teachername ||
      statusMeta?.statusObj?.details?.current?.teacher_name ||
      (Array.isArray(event.teachernames)
        ? event.teachernames[0]
        : event.teachernames || "New Teacher");
    ...
    if (summaryPrevious?.teacher?.name) {
      ...
      prevTeacherName =
        resPrev.teacher_name || resPrev.teachername || "Previous Teacher";
    ...
    const startTime =
      typeof event.start === "number" ? formatTime(event.start) : "";
  ...
  function renderMidnightIcon(event, options = {}) {
    ...
    return renderIcon(iconType.symbol, iconType.label, iconType.class, {
      isSymbol: true,
    });
  }
  ...
  function renderEventIcons(event, statusMeta = null, options = {}) {
    ...
    return {
      status: showStatusIcon
        ? renderStatusIcons(event, statusMeta, {
            ...statusIconOptions,
            hideForRescheduleCurrent: true,
          })
        : "",
      type: showTypeIcon
        ? renderTypeIcon(event, { isShortEvent, ...typeIconOptions })
        : "",
      teacherChange:
        showTeacherChangeIcon && event.isTeacherChanged
          ? renderTeacherChangeIcon(event, statusMeta)
          : "",
  ...
  function renderShortEventTypeIcon(event) {
    ...
    if (classType === "one2one_weekly" || classType === "one2one_single") {
      return renderIcon(
        TYPE_ICON_MAP.one2one_single.icon,
        TYPE_ICON_MAP.one2one_single.label,
        TYPE_ICON_MAP.one2one_single.class,
      );
    }
    ...
    if (event.isRescheduleCurrent && !event.isTeacherChanged) {
      return renderIcon(
        TYPE_ICON_MAP.makeup.icon,
        TYPE_ICON_MAP.makeup.label,
        TYPE_ICON_MAP.makeup.class,
      );
    }

    // Recurring events
    return renderIcon(
      TYPE_ICON_MAP.repeat.icon,
      TYPE_ICON_MAP.repeat.label,
      TYPE_ICON_MAP.repeat.class,
    );
  }
